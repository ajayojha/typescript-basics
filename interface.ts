...
export class RxSelectComponent extends RxHttp implements AfterContentInit, OnDestroy, OnChanges {
    ...
    constructor(public elementRef: ElementRef, public renderer: Renderer2) {
        ...
        let placeholderText = MultiLingualData.get("global.control.rx-select.placeholder");
        if (this.selectPlaceholder == "" && placeholderText)
            this.selectPlaceholder = placeholderText;
    ...
    @Input() valueInQueryParams: boolean;
    ...
    setValue(isBlur: boolean = false, columnName: string = "key"): boolean {
        if (!this.autoComplete && (this.currentValue !== undefined && this.currentValue !== "") && this.mainSource.length > 0) {
    ...
    ngOnChanges(source: any): void {
        ...
        if (this.focus)
            var t = setTimeout(() => {
                this.applyFocus(this.selectSearch.nativeElement);
            }, 1000)
    }
    ...
    private writeValue(value: any, fromNg: boolean = false) {
        if (value !== undefined) {
        ...
        if (this.formControl && !fromNg)
            this.formControl.setValue(this.currentValue);
    }

    caretClick(event: any): void {

        if (!this.selectDisabled && (this.activeTag == undefined || this.activeTag == "")) {
            var t = setTimeout(() => {
        ...
        if ((!this.userSource && this.mainSource.length === 0 && this.lookup) || (this.autoComplete) || this.isObjectChanged) {
            if ((this.currentValue && this.currentValue.length >= this.minimumCharacterSearchLength) || (!this.autoComplete)) {
                ...
                if (this.autoComplete && this.currentValue) {
                    if (this.valueInQueryParams) {
                        if (!this.queryParams)
                            this.queryParams = {};
                        this.queryParams['q'] = this.currentValue;
                    }
                    else
    ...
    showItemBox() {
        if (!this.currentValue)
            this.dropSource = this.mainSource;
    ...
    onBlur(event: any, value: string): void {
        if (this.currentFormControl)
            this.currentFormControl.markAsTouched();
        if (this.timeOutId)
    ...
    onKeyup(event: KeyboardEvent, scroller: HTMLElement): void {
        ...
        if (value !== null && event.key !== "Shift" && event.key !== "ArrowUp" && event.key !== "ArrowDown" && event.key !== "Enter" && event.key !== "ArrowLeft" && event.key !== "ArrowRight") {

            if (this.autoComplete) {
                this.currentValue = value;
                this.onFocus(event);
            } else {
                var regex = new RegExp(String(value).toLowerCase());
                this.dropSource = this.mainSource.filter(t => regex.test(String(t.value).toLowerCase()));
                this.setActiveTabClass();
            }
    ...
    onKeydown(event: KeyboardEvent, scroller: HTMLElement): void {
        ...
        else if (event.key === "ArrowUp" && this.dropSource.length > 0) {
            event.preventDefault();
    ...
    private changeScroll(offset?: number): void {
        ...
        if (scrollerElement) {
            ...
            if (childElement != undefined)
                scrollerElement.scrollTop = offset === 0 ? 0 : childElement.offsetTop - 50;
        }
    }
    ...
    onMouseover(tag?: SelectModel, i?: number) {
