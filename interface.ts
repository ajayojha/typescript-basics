...
export class RxTagComponent implements AfterContentInit, OnDestroy, OnChanges {
    ...
    constructor(public elementRef: ElementRef, public renderer: Renderer2) {
        ...
        this.maxSelectionMessage = MultiLingualData.get("global.control.rx-tag.message-max-selection");
        this.splitCharacter = MultiLingualData.get("global.control.rx-tag.split-character");
        let sortConfiguration: any = MultiLingualData.get("global.control.rx-tag.is-sort");
        this.isSort = (sortConfiguration === '') ? this.isSort : sortConfiguration;
        this.placeholder = this.placeholder || MultiLingualData.get("global.control.rx-tag.placeholder");
    ...
    ngAfterContentInit(): void {
        ...
        else if (this.itemTexts && (this.currentValue !== undefined && this.currentValue !== "")) {
            var selectedItemSource = [];
            var splitIds = String(this.currentValue).split(this.splitCharacter);
    ...
    private sortSource() {
        this.dropSource = this.isSort === undefined || this.isSort === true || String(this.isSort) == "true" ? this.dropSource.sort(this.customSort((t: TagModel) => t.text, false)) : this.dropSource;
    }

    private customSort(predicate: (key: TagModel) => any, orderByDescending: boolean) {
        return (a: any, b: any) => {
    ...
    private setSelectedTags(source: Array<TagModel>, tags: string): void {
        this.selectedTags = new Array<TagModel>();
        if (tags)
            String(tags).split(this.splitCharacter).forEach(v => {
                if (v != "") {
                    var baseId: any = v;
